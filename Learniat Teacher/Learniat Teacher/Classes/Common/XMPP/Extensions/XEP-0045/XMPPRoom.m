...
{
    [self joinRoomUsingNickname:desiredNickname history:history password:nil withPrivilage:[NSXMLElement elementWithName:@"x" xmlns:XMPPMUCNamespace]];
}

- (void)joinRoomUsingNickname:(NSString *)desiredNickname history:(NSXMLElement *)history password:(NSString *)passwd withPrivilage:(NSXMLElement*)element
{
    dispatch_block_t block = ^{ @autoreleasepool {
        ...
        if (history)
        {
            [element addChild:history];
        }
        if (passwd)
        {
            [element addChild:[NSXMLElement elementWithName:@"password" stringValue:passwd]];
        }
