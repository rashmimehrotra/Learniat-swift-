...
class SSStudentSchedulePopoverController: UIViewController,SSStudentDataSourceDelegate
{
    ...
    override func viewDidLoad()
    {
        ...
        mDoneButton.addTarget(self, action: #selector(SSStudentSchedulePopoverController.onDoneButton), forControlEvents: UIControlEvents.TouchUpInside)
        ...
        timer = NSTimer.scheduledTimerWithTimeInterval(60, target: self, selector: #selector(SSStudentSchedulePopoverController.timerAction), userInfo: nil, repeats: true)
    ...
    func onEndSession()
    {
        ...
        if delegate().respondsToSelector(#selector(SSStudentSchedulePopoverControllerDelegate.delegateSessionEnded))
        {
