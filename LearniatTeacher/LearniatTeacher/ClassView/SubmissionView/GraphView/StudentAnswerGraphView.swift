...
@objc protocol StudentAnswerGraphViewDelegate
{
    ...
    @objc optional func delegateShareButtonClickedWithDetails(_ details:AnyObject)
...
class StudentAnswerGraphView: UIView
{
    ...
    override init(frame: CGRect)
    {
        ...
        shareGraphButton.frame = CGRect(x: self.frame.size.width - 105, y: 0, width: 100, height: 50)
        shareGraphButton.setImage(UIImage(named:"Sharebutton.png"), for:UIControlState())
    ...
    func loadMTCViewWithOPtions(_ optionsArray:NSMutableArray,WithRightSideOptionArray rightOPtionsArray:NSMutableArray, withQuestion questionName:String)
    {
        ...
        for index in 0 ..< optionsArray.count
        {
            ...
            lineContainerView.addSubview(barView)
            barView.addTarget(self, action: #selector(StudentAnswerGraphView.onBarButtonPressed(_:)), for: UIControlEvents.touchUpInside)
            if let optionId = (rightOPtionDict as AnyObject).object(forKey: "OptionId") as? String
            {
                barView.tag = Int(optionId)!
    ...
    func onShareGraph()
    {
        ...
        for subview in subViews
        {
            if subview.isKind(of: BarView.self)
            {
                ...
                detailsDictonary.setObject(roundOffNumberWithFloat(Float(valueOfBars)), forKey: "option_\(optionsId)" as NSCopying)
        ...
        if delegate().responds(to: #selector(StudentAnswerGraphViewDelegate.delegateShareButtonClickedWithDetails(_:)))
        {
